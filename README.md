...
| Construct | Meaning |
|---|---|
| `auth` | built-in users: `signup`/`login`/`logout`, `actor`, `role` |
| `entity Name:` | durable record type; a field may reference another entity |
| `state n: T = v [@client]` | a state cell; authoritative unless `@client` |
| `derive n: T = expr` | a named computed value (inlined, reactive) |
| `policy n:` | a boolean predicate over `actor`/`role`/state |
| `action n(p):` | a mutation: `assign`/`add`/`set`/`remove`/`clear`, `requires` |
...
`examples/social.fct` (auth + pages + feed), `examples/chirp.fct`,
`examples/inbox.fct` (relations + jobs), `examples/counter.fct` (the minimal
...
v1.0.0 — a real, usable foundation: data (Postgres) · queries · auth · pages ·
projections · packaging, end to end. See [ENTERPRISE.md](ENTERPRISE.md) for the
roadmap to large-scale/regulated production. Each item is another node kind or
runtime service through the **same IR and placement calculus** — the language
grows by addition, never by forking into frontend and backend again.
```sh
go test ./...   # placement soundness, queries, auth, pages, the evaluator
```
